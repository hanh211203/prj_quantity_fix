...
{
	$id 	= filter_var($_POST["prd_id"], FILTER_SANITIZE_STRING); //product code
	...
	$results = $mysqli->query("SELECT prd_name,price FROM product WHERE prd_id='$id' LIMIT 1");
	
	if ($results) { //we have the product info 
		$obj = $results->fetch_object();
		//prepare array for the session variable
		$new_product = array(array('name'=>$obj->name, 'id'=>$id, 'quantity'=>$quantity, 'price'=>$obj->price, 'sale'=>$sale));
